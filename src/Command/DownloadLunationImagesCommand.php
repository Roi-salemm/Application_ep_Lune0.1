...
#[AsCommand(
    name: 'app:moon:download-lunation',
    description: 'Telecharge les images NASA SVS d une lunaison complete (nouvelle lune -> nouvelle lune) et les renomme avec un degre 0..360.'
)]
class DownloadLunationImagesCommand extends Command
{
    private const YEAR = 2020;
    private const LUNATION_INDEX = 0;
    private const STEP_HOURS = 2;
    private const OUT_DIR = 'var/moon/lunaison_2h';
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $csv = fopen($csvPath, 'w');
        fputcsv($csv, ['frame','filename','utc_iso','age_days','phase_percent','deg_0_360']);
        ...
        foreach ($selected as $idx) {
            ...
            $deg = 360.0 * (($row['dt']->getTimestamp() - $t0->getTimestamp()) / $durationSec);
            if ($deg < 0) {
                $deg = 0.0;
            }
            if ($deg > 360) {
            ...
            $dstName = sprintf('moon_f%04d_deg%06.1f.jpg', $frame, $deg);
        ...
        // Note importante : a 2h, tu obtiens typiquement ~350-370 images, pas 400.
        $output->writeln('<info>Note: avec un pas de 2h, une lunaison (~708h) donne ~354 images en moyenne.</info>');
